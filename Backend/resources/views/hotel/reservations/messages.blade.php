...
@section('title', 'Messages — Réservation ' . $reservation->code_reservation . ' - EVADIA')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route('hotel.reservations.show', $reservation->id) }}" class="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">Réservation {{ $reservation->code_reservation }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="h-[500px] overflow-y-auto p-6 space-y-4" id="messages-container">
            @forelse($messages as $message)
                @php $isMe = $message->expediteur_id === auth('hotel')->id(); @endphp
            ...
            @empty
                <div class="text-center py-12">
                    <p class="text-sm text-gray-400">Aucun message pour cette réservation</p>
                </div>
            @endforelse
        </div>
        ...
        <div class="border-t border-gray-200 p-4">
            <form method="POST" action="{{ route('hotel.reservations.messages.store', $reservation->id) }}" class="flex gap-3">
                @csrf
                <textarea name="contenu" required rows="1" placeholder="Votre message au client..."
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.Echo) {
        window.Echo.private('messages.{{ auth("hotel")->id() }}')
            .listen('.message.sent', (e) => {
                if (e.expediteur_id == {{ $reservation->client_id }}) {
                    const msgHtml = `
