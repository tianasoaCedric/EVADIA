...
@section('content')
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">Connectez-vous à votre espace hôtelier</p>
    </div>
        ...
        <div class="flex items-center justify-between">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="remember"
                    class="h-4 w-4 rounded border-gray-300 text-hotel-600 focus:ring-hotel-500 cursor-pointer">
                <span class="text-sm text-gray-600">Se souvenir de moi</span>
            </label>
            <a href="{{ route('hotel.password.request') }}" class="text-sm text-hotel-600 hover:text-hotel-700 font-medium transition-colors">
                Mot de passe oublié ?
            </a>
        </div>

        <button type="submit" id="hotel-login-submit"
            class="w-full rounded-xl bg-hotel-600 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-hotel-700 focus:outline-none focus:ring-2 focus:ring-hotel-500 focus:ring-offset-2 transition-all duration-200 active:scale-[0.98]">
            Se connecter
        </button>
    </form>
