...
@section('title', 'Notifications - EVADIA')
...
<div class="max-w-3xl mx-auto space-y-4">
    ...
    <div class="flex items-center justify-between">
        ...
        @if($notifications->where('lu', false)->count() > 0)
            <form method="POST" action="{{ route('hotel.notifications.mark-all-read') }}">
                @csrf
                <button type="submit" class="text-sm text-hotel-600 hover:text-hotel-700 font-medium">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden divide-y divide-gray-50">
        @forelse($notifications as $notif)
            <div class="flex gap-4 px-5 py-4 {{ !$notif->lu ? 'bg-hotel-50/30' : '' }}">
                {{-- Icon --}}
                <div class="shrink-0 mt-0.5">
                    <div class="h-10 w-10 rounded-full flex items-center justify-center {{ !$notif->lu ? 'bg-hotel-100 text-hotel-600' : 'bg-gray-100 text-gray-400' }}">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        @if(!$notif->lu)
                            <span class="h-2.5 w-2.5 rounded-full bg-hotel-500 shrink-0 mt-1.5"></span>
                        @endif
                    </div>
                    <div class="flex items-center gap-3 mt-2">
                        <span class="text-xs text-gray-400">{{ $notif->date_envoi?->diffForHumans() }}</span>
                        @if($notif->lien)
                            <a href="{{ $notif->lien }}" class="text-xs text-hotel-600 hover:text-hotel-700 font-medium">Voir</a>
                        @endif
                        @if(!$notif->lu)
                            <form method="POST" action="{{ route('hotel.notifications.mark-read', $notif) }}" class="inline">
                                @csrf @method('PATCH')
